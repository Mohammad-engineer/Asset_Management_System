...
export const USER_STATUS_OPTIONS = [
  ...
  { value: 'تعمیر', label: 'تعمیر' },
  { value: 'اسقاط', label: 'اسقاط' },
  { value: 'از رده خارج شده', label: 'از رده خارج شده' },
];
...
export const _userList = [...Array(10)].map((_, index) => ({
  ...
  company: _mock.companyNames(0),
  country: _mock.countryNames(index),
  avatarUrl: _mock.image.avatar(index),
  phoneNumber: _mock.phoneNumber(index),
  status: (index % 2 && 'اسقاط') || (index % 3 && 'تعمیر') || (index % 4 && 'از رده خارج') || 'فعال',
  ...
  codeSabt: (Math.random(1,100)*1000).toFixed(0),
  codeRahgiry: (Math.random(1,500)*5000).toFixed(0),
  desc: 'توضیحات',
  vaziat: 'ثبت اولیه',
}));
